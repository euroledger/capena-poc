...
class FaberAgent(DemoAgent):
    ...
    async def handle_issue_credential(self, message):
        ...
        if state == "request_received":
            ...
            #     message["credential_definition_id"],
            #     ", cred_attrs =",
            #     cred_attrs,
            # )
            # cred_preview = {
...
async def main(
    ...
    try:
        ...
        with log_timer("Publish schema/cred def duration:"):
            ...
            #Create ETSY Schema, Cred def
            (
                etsy_schema_id,  # schema id
                etsy_cred_def_id,
            ) = await agent.register_schema_and_creddef(
                "faber_etsy_cred_def",
                "faber_etsy_schema",
                version,
                ["etsy_name", "etsy_feedbackcount", "etsy_registrationdate", "etsy_posfeedbackpercent"],
                support_revocation=revocation,
            )

            with log_timer("Publish Etsy revocation registry duration:"):
                log_status(
                    "#5/6 Create and publish the ETSY revocation registry on the ledger"
                )
                await agent.create_and_publish_revocation_registry(
                    etsy_cred_def_id, TAILS_FILE_COUNT
                )
            #Create Ebay Schema, Cred def
            (
                ebay_schema_id,  # schema id
                ebay_cred_def_id,
            ) = await agent.register_schema_and_creddef(
                "faber_ebay_cred_def",
                "faber_ebay_schema",
                version,
                ["name", "feedbackscore", "registrationdate", "negfeedbackcount", "posfeedbackcount","posfeedbackpercent"],
                support_revocation=revocation,
            )
            with log_timer("Publish Ebay revocation registry duration:"):
                log_status(
                    "#5/6 Create and publish the EBAY revocation registry on the ledger"
                )
                await agent.create_and_publish_revocation_registry(
                    ebay_cred_def_id, TAILS_FILE_COUNT
                )
